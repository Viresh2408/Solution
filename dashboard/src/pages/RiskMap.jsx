...
import { useGlobalThreats } from '../hooks/useTrustNetData';
...
export default function RiskMap() {
  ...
  const { threats: globalThreats, loading } = useGlobalThreats();

  const attackTypes = ['All', 'Phishing', 'BEC', 'Vishing', 'Smishing', 'APT', 'Multiple'];

  useEffect(() => {
    if (loading || globalThreats.length === 0) return;
      ...
      globalThreats.forEach(threat => {
        ...
        marker.bindPopup(`
          <div style="background:#0D1117;color:#E8EAF0;font-family:Inter,sans-serif;padding:8px;min-width:160px;border-radius:8px;">
            <div style="font-weight:700;font-size:13px;margin-bottom:6px">${threat.country || 'Unknown'}</div>
            <div style="font-size:11px;color:#9BA3B2">Attack Type: <strong style="color:${color}">${threat.type || 'Phishing'}</strong></div>
            <div style="font-size:11px;color:#9BA3B2">Intensity: <strong style="color:${color}">${threat.intensity || 0}/100</strong></div>
            <div style="font-size:11px;color:#9BA3B2">Incidents: <strong>${(threat.count || 0).toLocaleString()}</strong></div>
          </div>
        `, {
    ...
    <div className="page">
      ...
      <div style={{display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', flexWrap:'wrap'}}>
        ...
        <div style={{display:'flex', alignItems:'center', gap:'6px', background:'rgba(255,71,87,0.1)', border:'1px solid rgba(255,71,87,0.3)', borderRadius:'100px', padding:'5px 14px', fontSize:'0.78rem'}}>
          ...
          <span style={{color:'var(--text-2)'}}>{(liveCount || 0).toLocaleString()} events today</span>
        </div>
      ...
      <div className="grid-2">
          ...
          <table className="data-table">
            ...
            <tbody>
              {sorted.slice(0, 8).map((t, i) => {
                const color = (t.intensity || 0) >= 85 ? 'var(--danger)' : (t.intensity || 0) >= 70 ? 'var(--warning)' : 'var(--cyan)';
                return (
                  <tr key={t.country || i} onClick={() => setSelected(t)} style={{cursor:'pointer'}}>
                    <td style={{color:'var(--text-3)', fontFamily:'var(--mono)'}}>{i + 1}</td>
                    <td style={{fontWeight:600, color:'var(--text)'}}>{t.country || 'Unknown'}</td>
                    <td><span className="badge danger">{t.type || 'Phishing'}</span></td>
                    <td>
                      <div className="flex items-center gap-2">
                        <div style={{width:'50px'}}>
                          <div className="progress-bar">
                            <div className="progress-fill" style={{width:`${t.intensity || 0}%`, background:color}}></div>
                          </div>
                        </div>
                        <span style={{fontFamily:'var(--mono)', fontSize:'0.72rem', color}}>{t.intensity || 0}</span>
                      </div>
                    </td>
                    <td style={{fontFamily:'var(--mono)'}}>{(t.count || 0).toLocaleString()}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="card">
          {selected ? (
            <>
              <div className="section-header mb-4">
                <div className="section-title">🌍 {selected.country || 'Unknown'}</div>
              ...
              <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'16px'}}>
                {[
                  { label:'Threat Intensity', val:(selected.intensity || 0) + '/100', color: (selected.intensity || 0) >= 75 ? 'var(--danger)' : 'var(--warning)' },
                  { label:'Primary Attack', val:selected.type || 'Phishing', color:'var(--danger)' },
                  { label:'Confirmed Incidents', val:(selected.count || 0).toLocaleString(), color:'var(--warning)' },
                  { label:'FL Devices Active', val:Math.round((selected.count || 0) * 0.3).toLocaleString(), color:'var(--cyan)' },
                ].map(item => (
