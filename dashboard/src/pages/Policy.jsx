...
export default function Policy() {
  ...
  const fetchOverrideRequests = async () => {
    ...
    try {
      ...
      setOverrideRequests(data || []);
    } catch { setOverrideRequests([]); }
  };

  useEffect(() => {
    fetchOverrideRequests();
    const ch = supabase.channel('override-queue')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'override_requests' }, fetchOverrideRequests)
      .subscribe();
  ...
  const handleOverride = async (id, action) => {
    setProcessingId(id);
    try {
      await supabase.from('override_requests').update({ status: action }).eq('id', id);
      const req = overrideRequests.find(r => r.id === id);
      console.log(`[POLICY] ${action}=true user=${req?.employeeName?.split('@')[0]} domain=${req?.domain}`);
    } catch {}
    setProcessingId(null);
    fetchOverrideRequests();
  };

  // F3 — Generate Weekly Report
  ...
  const generateWeeklyReport = async () => {
    ...
    setReportUrl(null);
    try {
      // Aggregate Supabase data
      const orgId = profile?.orgId || 'apex-financial';
      const [alertsRes, modulesRes] = await Promise.allSettled([
        supabase.from('alerts').select('type, severity, blocked, status').eq('orgId', orgId),
        supabase.from('trainingModules').select('title, enrolled, completed, avgScore').eq('orgId', orgId),
      ]);

      const alerts  = alertsRes.value?.data  || [];
      const modules = modulesRes.value?.data || [];
      ...
      const completionPct = modules.length
        ? Math.round(modules.reduce((s, m) => s + (m.completed || 0), 0) / Math.max(1, modules.reduce((s, m) => s + (m.enrolled || 0), 0)) * 100)
        : 0;

      console.log(`[REPORT] org=${orgId} generated_at=${reportDate} threats=${totalAlerts}`);

      const md = `# TrustNet AI — Weekly Security Report
**Organization:** ${orgId}  
**Report Period:** ${reportDate}  
**Generated by:** TrustNet AI (Gemini 1.5 Flash)

---

## Executive Summary

During this reporting period, TrustNet AI monitored all employee sessions and blocked **${blocked}** out of **${totalAlerts}** threats automatically. There were **${criticals}** critical-severity incidents requiring attention. Training completion stands at **${completionPct}%**.
...
| Critical Incidents | ${criticals} |
| High Severity | ${alerts.filter(a=>a.severity==='high').length} |

## Training Completion

${modules.map(m => `- **${m.title}**: ${m.completed}/${m.enrolled} (${Math.round((m.completed/Math.max(1,m.enrolled))*100)}%)`).join('\n') || '- No modules found.'}

---
*Report generated by TrustNet AI on ${new Date().toISOString()}*`;
  ...
  return (
    <div className="page">
      ...
      {/* F7 — Zero-Trust Override Queue */}
      <div className="card mb-4">
        <div className="section-header mb-4">
          <div>
            <div className="section-title">🔒 Zero-Trust Override Queue</div>
            <div className="section-subtitle">Employees blocked from submitting credentials on high-risk pages</div>
          </div>
          <span style={{fontFamily:'var(--mono)', fontSize:'0.8rem', color:'var(--warning)', fontWeight:700}}>
            {overrideRequests.length} pending
          </span>
        </div>

        {overrideRequests.length === 0 ? (
          <div style={{textAlign:'center', padding:'28px', color:'var(--text-3)', fontSize:'0.85rem'}}>
            ✓ No pending override requests — Zero-Trust policy holding
          </div>
        ) : (
          <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
            {overrideRequests.map(req => (
              <div key={req.id} style={{
                ...
                padding:'14px 16px', border:'1px solid rgba(255,165,2,0.2)',
              }}>
                <div>
                  <div style={{fontWeight:600, fontSize:'0.85rem'}}>{req.employeeName}</div>
                  <div style={{fontSize:'0.72rem', color:'var(--text-3)', fontFamily:'var(--mono)'}}>
                    {new Date(req.requestedAt).toLocaleTimeString()}
                  </div>
                </div>
                <div>
                  <div style={{fontSize:'0.8rem', color:'var(--text-2)', fontFamily:'var(--mono)'}}>{req.domain}</div>
                  <div style={{display:'flex', alignItems:'center', gap:'4px', marginTop:'3px'}}>
                    <div style={{height:3, width:60, background:'var(--bg-card)', borderRadius:2}}>
                      <div style={{height:'100%', width:(req.riskScore||0)+'%', background:(req.riskScore||0)>=75?'var(--danger)':'var(--warning)', borderRadius:2}}/>
                    </div>
                    <span style={{fontSize:'0.65rem', color:'var(--text-3)'}}>{req.riskScore}/100</span>
                  </div>
                </div>
                <button disabled={processingId===req.id} onClick={() => handleOverride(req.id,'approved')}
                  className="btn btn-primary btn-sm"
                  style={{background:'var(--safe)', borderColor:'transparent', fontSize:'0.72rem'}}>
                  {processingId===req.id ? '…' : '✓ Approve'}
                </button>
                <button disabled={processingId===req.id} onClick={() => handleOverride(req.id,'denied')}
                  className="btn btn-ghost btn-sm"
                  style={{color:'var(--danger)', borderColor:'rgba(255,71,87,0.3)', fontSize:'0.72rem'}}>
                  ✕ Deny
                </button>
              </div>
            ))}
      ...
      <div className="card">
        <div className="section-header mb-4">
          ...
          <div style={{display:'flex', gap:'8px', alignItems:'center'}}>
            {reportUrl && (
              <a href={reportUrl} download={`trustnet-report-${new Date().toISOString().slice(0,10)}.md`}
                className="btn btn-primary btn-sm"
                style={{color:'var(--bg-base)', background:'var(--safe)', borderColor:'transparent', textDecoration:'none'}}>
                ⬇ Download Report
              </a>
            )}
            <button className="btn btn-ghost btn-sm" onClick={generateWeeklyReport} disabled={generatingReport} style={{fontSize:'0.78rem'}}>
